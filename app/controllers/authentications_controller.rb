...
def create
  ...
  if uid != '' and provider != ''
      ...
      if !auth.nil?
        ...
        @current_user.authentications.create(:provider => provider, :uid => uid, :name => name, :email => email, :first_name => first_name, :last_name => last_name, :image_path => image_path, :location_name => location_name, :location_id => location_id, :token => omniauth['credentials']['token'])
        flash[:notice] = "Facebook authentication successful"
        
        return
      else  
        user = User.find_or_initialize_by_email(:email => email)
        auth_hash = {:provider => provider, :uid => uid, :name => name, :email => email, :first_name => first_name, :last_name => last_name, :image_path => image_path, :location_name => location_name, :location_id => location_id, :token => omniauth['credentials']['token']}
